  ...
  template: `
    <div class="chart-wrap">
      <svg [attr.viewBox]="'0 0 ' + width + ' ' + height" class="chart-svg">
        ...
        <circle *ngFor="let p of points; let i = index" [attr.cx]="p.x" [attr.cy]="p.y" r="4" [attr.fill]="color" class="chart-dot" stroke="var(--bg-card)" stroke-width="1.5"/>
      </svg>
    ...
    <div class="donut-wrap">
      <svg [attr.viewBox]="'0 0 120 120'" class="donut-svg">
        ...
        <circle *ngFor="let s of segments; let i = index"
          ...
          style="transform-origin: center; transform: rotate(-90deg); transition: stroke-dasharray 1.2s ease, stroke-dashoffset 1.2s ease;"
        />
        <circle *ngFor="let s of segments; let i = index"
          cx="60" cy="60" r="45" fill="none"
          [attr.stroke]="s.color"
  ...
  template: `
    <div class="dist-wrap" *ngIf="bars.length">
      <div class="bar-row" *ngFor="let b of bars">
        ...
        <div class="bar-track"><div class="bar-fill" [style.width.%]="b.pct" [style.background]="b.color"><span class="bar-val">{{ b.count }}</span></div></div>
      </div>
    </div>
  `,
  styles: [`
